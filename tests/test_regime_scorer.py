...
class TestMarketRegimeScorer(unittest.TestCase):
    ...
    def _create_mock_df(self, count=30, vol_mult=1.0, atr=2.5):
        ...
        for i in range(count):
            vol = 1000 * (vol_mult if i >= count - 2 else 1.0)
            data.append({
    ...
    def test_prime_confluence_generates_grade_a_plus(self):
        # 15:30 London session (14:00 - 19:00 TH), 1.8x volume surge, healthy ATR 3.0, tight spread 12 pts
        df = self._create_mock_df(count=30, vol_mult=1.8, atr=3.0)
        eval_time = datetime(2026, 8, 31, 15, 30) # London Session (14:00 - 19:00 Thai Time)
    ...
    def test_rollover_illiquid_generates_grade_c_and_blocks(self):
        # 05:30 Rollover Deadzone (04:00 - 07:00 TH), 0.3x thin volume, low ATR, widened spread 55 pts
        df = self._create_mock_df(count=30, vol_mult=0.3, atr=0.5)
        eval_time = datetime(2026, 8, 31, 5, 30) # Rollover Deadzone
        res = self.scorer.evaluate_market_confluence(df, current_spread=55.0, setup_id="EMA50_3CANDLES_H1", server_time=eval_time)
    ...
    def test_asian_range_sniper_rewards_asian_session(self):
        # 09:00 Asian Session (07:00 - 14:00 TH), normal volume, calm ATR 1.5
        df = self._create_mock_df(count=30, vol_mult=1.1, atr=1.5)
        eval_time = datetime(2026, 8, 31, 9, 0)
